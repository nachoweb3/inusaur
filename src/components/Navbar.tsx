    ...
    >
      ...
      >
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <ConnectWalletButton />
          <Button href={isPlaceholder(config.buyUrl) ? undefined : config.buyUrl!} showSoon={isPlaceholder(config.buyUrl)} className="px-5 py-2.5">
            {isPlaceholder(config.buyUrl) ? config.microcopy.awaitingGenesis : `BUY ${config.ticker}`}
          </Button>
        </div>
      ...
      >
        <div className="container-x flex flex-1 flex-col justify-center gap-2 pt-24">
          ...
          >
            ...
            <Button href={isPlaceholder(config.buyUrl) ? undefined : config.buyUrl!} showSoon={isPlaceholder(config.buyUrl)} className="w-full py-4">
              {isPlaceholder(config.buyUrl) ? config.microcopy.awaitingGenesis : `BUY ${config.ticker}`}
            </Button>
          </div>
